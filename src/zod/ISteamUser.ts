...
export const playerSummarySchema = z.object({
     ...
     */
    steamid: z.string().regex(/^[0-9]{17}$/).describe("The player's 64-bit SteamID."),
    /**
     * @minimum 0
     * @maximum 5
     ...
     */
    communityvisibilitystate: z.number().min(0).max(5).describe("Player's community visibility state (0 = Private, 1 = Friends Only, 2 = Friends of Friends, 3 = Users Only, 4 = Public, 5 = All)."),
    /**
     * @minimum 0
     * @maximum 1
     * @description If the profile is not visible, this will be 0. If the profile is visible, this will be 1.
     */
    profilestate: z.number().min(0).max(1).optional().describe("If the profile is not visible, this will be 0. If the profile is visible, this will be 1."),
    /**
     * @minLength 1
     * @maxLength 32
     * @description The player's persona name (display name).
     */
    personaname: z.string().min(1).max(32).describe("The player's persona name (display name)."),
    /**
     * @format unix-timestamp
     * @description The last time the player was logged off Steam.
     */
    lastlogoff: z.number().describe("The last time the player was logged off Steam."),
    /**
     * @format url
     * @description The URL to the player's Steam Community profile.
     */
    profileurl: z.string().url().describe("The URL to the player's Steam Community profile."),
    /**
     * @format url
     ...
     */
    avatarmedium: z.string().url().describe("The URL to the player's 64x64px avatar."),
    /**
     * @format url
     * @description The URL to the player's 184x184px avatar.
     */
    avatarfull: z.string().url().describe("The URL to the player's 184x184px avatar."),
    /**
     * @minimum 0
     * @maximum 6
     ...
     */
    personastate: z.number().min(0).max(6).describe("The player's current online status (0 = Offline, 1 = Online, 2 = Busy, 3 = Away, 4 = Snooze, 5 = Looking to Trade, 6 = Looking to Play)."),
    /**
     * @pattern ^[0-9]{17}$
     * @description The 64-bit ID of the player's primary clan.
     */
    primaryclanid: z.string().regex(/^[0-9]{17}$/).optional().describe("The 64-bit ID of the player's primary clan."),
    /**
     * @format unix-timestamp
     * @description The time the player's Steam account was created.
     */
    timecreated: z.number().optional().describe("The time the player's Steam account was created."),
    /**
     * @minLength 1
     ...
     */
    loccountrycode: z.string().min(2).max(2).optional().describe("The player's country code."),
    /**
     * @minLength 2
     * @maxLength 2
     * @description The player's state code.
     */
    locstatecode: z.string().min(2).max(2).optional().describe("The player's state code."),
    /**
...
export const checkAppOwnershipResponseSchema = z.object({
    appownership: z.object({
         ...
         */
        ownsApp: z.boolean().describe("True if the user owns the app, false otherwise."),
        /**
         * @format unix-timestamp
         * @description The time the app was acquired by the user.
         */
        timeAcquired: z.number().describe("The time the app was acquired by the user."),
        /**
         * @pattern ^[0-9]{17}$
         * @description The 64-bit SteamID of the true owner (if family shared).
         */
        ownerSteamID: z.string().regex(/^[0-9]{17}$/).describe("The 64-bit SteamID of the true owner (if family shared)."),
        /**
         * @description True if the app is accessed via site license (PC Cafe program).
         */
        sitelicense: z.boolean().optional().describe("True if the app is accessed via site license (PC Cafe program).")
    })
});
...
export const getDeletedSteamIDsRequestSchema = z.object({
     ...
     */
    rowversion: z.string().regex(/^[0-9]+$/).describe("An unsigned 64-bit value for pagination.")
});
...
export const deletedSteamIDSchema = z.object({
     ...
     */
    steamid: z.string().regex(/^[0-9]{17}$/).describe("The 64-bit SteamID of the deleted account.")
});
...
export const getDeletedSteamIDsResponseSchema = z.object({
    response: z.object({
         ...
         */
        rowversion: z.string().regex(/^[0-9]+$/).describe("The rowversion for the next request.")
    })
});
...
export const userGroupSchema = z.object({
     ...
     */
    gid: z.string().regex(/^[0-9]+$/).describe("The 64-bit ID of the group.")
});
...
export const resolveVanityURLRequestSchema = z.object({
     ...
     */
    url_type: z.number().min(1).max(3).optional().describe("The type of vanity URL (1 = individual, 2 = group, 3 = game group).")
});
...
export const resolveVanityURLResponseSchema = z.object({
    response: z.object({
         ...
         */
        steamid: z.string().regex(/^[0-9]{17}$/).optional().describe("The 64-bit SteamID if resolved successfully."),
        /**
         * @minimum 1
         * @description The success status (1 = success, 42 = no match).
         */
        success: z.number().min(1).describe("The success status (1 = success, 42 = no match)."),
        /**
...
export const getFriendListRequestSchema = z.object({
     ...
     */
    relationship: z.string().regex(/^(all|friend)$/).optional().describe("Filter by relationship type (all or friend).")
});
...
export const friendSchema = z.object({
     ...
     */
    steamid: z.string().regex(/^[0-9]{17}$/).describe("The 64-bit SteamID of the friend."),
    /**
     * @pattern ^(friend)$
     * @description The relationship type (always 'friend').
     */
    relationship: z.string().regex(/^(friend)$/).describe("The relationship type (always 'friend')."),
    /**
     * @format unix-timestamp
     * @description The Unix timestamp of when the friendship was created.
     */
    friend_since: z.number().describe("The Unix timestamp of when the friendship was created.")
});
...
export const getPlayerBansRequestSchema = z.object({
     ...
     */
    steamids: z.string().regex(/^[0-9,]+$/).describe("Comma-separated list of 64-bit SteamIDs.")
});
...
export const playerBanSchema = z.object({
     ...
     */
    CommunityBanned: z.boolean().describe("True if the player is banned from the Steam Community."),
    /**
     ...
     */
    NumberOfVACBans: z.number().min(0).describe("The number of VAC bans the player has."),
    /**
     * @minimum 0
     * @description The number of days since the player's last ban.
     */
    DaysSinceLastBan: z.number().min(0).describe("The number of days since the player's last ban."),
    /**
     * @pattern ^(none|probation|banned)$
     * @description The player's economy ban status.
     */
    EconomyBan: z.string().regex(/^(none|probation|banned)$/).describe("The player's economy ban status.")
});
